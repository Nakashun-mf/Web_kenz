...
export function InspectionPage() {
    ...
    return (
      <>
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-10 bg-slate-50 px-8">
          ...
          <FileDropzone onFile={handleFile} className="w-full max-w-xs" />
          {loading && (
            <div className="flex items-center gap-2.5 rounded-xl bg-indigo-50 px-4 py-2.5 text-sm font-medium text-indigo-600">
              <div className="h-4 w-4 animate-spin rounded-full border-2 border-indigo-600 border-t-transparent" />
              読み込み中…
            </div>
          )}
          {error && (
            <p className="rounded-xl bg-red-50 px-4 py-2 text-sm font-medium text-red-500">{error}</p>
          )}
        </div>
      </>
    )
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* ── File info bar ── */}
      <div className="flex h-10 shrink-0 items-center gap-2.5 border-b border-slate-200 bg-white px-3">
        <FileText className="h-3.5 w-3.5 shrink-0 text-slate-400" />
        <span
          className="truncate text-xs font-semibold text-slate-700 max-w-[260px]"
          title={file.name}
        >
          {file.name}
        </span>
        ...
        <button
          ...
          className="rounded-lg px-2.5 py-1 text-xs font-medium text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
        >
          変更
        </button>
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-medium text-indigo-500 transition-colors hover:bg-indigo-50 hover:text-indigo-700"
        >
          比較モードへ送る
